...
const ${capitalize(moduleName)} = model<I${capitalize(
      moduleName,
    )}>("${capitalize(moduleName)}", ${moduleName}Schema);
...
const updateUserProfile = async (id: string, payload: Partial<I${capitalize(
    ...
    const user = await ${capitalize(moduleName)}.findById(id);
    if (!user) {
        throw new AppError(httpStatus.NOT_FOUND, "Profile not found");
    }
    return await ${capitalize(moduleName)}.findByIdAndUpdate(id, payload, {
        new: true,
        runValidators: true,
    });
};
